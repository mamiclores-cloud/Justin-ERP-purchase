...
function allocate(specs, lowSales = DEFAULT_LOW_SALES) {
  ...
  // 2) 彈性:分給「最需要湊低銷」(離低銷差距最大且尚未達標)的候選廠商
  flexible.forEach((s) => {
    let best = null, bestGap = -Infinity;
    cand.get(s.key).forEach((v) => {
      const gap = lowSales[v] - totals[v];   // >0 = 還沒達標,越大越需要
  ...
  while (changed && guard++ < 100) {
    ...
    for (const uv of VENDORS) {
      ...
      for (const s of movable) {
        const alt = cand.get(s.key).find((v) => v !== uv && totals[v] >= lowSales[v]);
