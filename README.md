...
| **1688** | `keywordType='ALL'` 廣泛 | 規格加總 ≥ 3 | `special` 標籤 / 貨號開頭 `KDS`（全域）＋ `Indo`/`TW`/`YLL`/`Thai`/`SKSP*` | **Phase 2 SKSP 共同採購**：同 `SKSP###` 代碼合單；**6天內不重複採購**：同商品在「採購中」6 天內已建單 → 跳過（固定 6 天） |
...
- 天數**固定 6 天**，不開放前端／排程調整；CLI `--recent-days N` 僅開發測試用（`0` = 關閉檢查）
...
flowchart TD
    ...
    Join --> Alloc[分配演算法<br/>6倍數/補最低量 · BOX分廠訂整箱<br/>多家有貨選一家:最低量少>單價低>低銷IL.IN.HS<br/>低銷 IL8000 HS3000 IN5000 · 需求全訂 · 湊不滿改別家]
...
| 分配選擇 | 需求全訂（低銷只是出貨門檻）；多家有貨**只給一家**,優先序：**① 採購量最少(最低採購量低→最不超量) ② 單價最便宜(缺價排最後) ③ 低銷分配:未達標優先、IL>IN>HS**；某家湊不滿低銷 → 改分給別家有貨;都沒人接 → 記異常 |
