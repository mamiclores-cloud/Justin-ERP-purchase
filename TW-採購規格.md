...
> **execute 完整度**:Phase B execute 會 ① 回填 sheet(`tw/sheet_writeback.py` 寫 需求量 + 各廠商採購量到**最右既有欄組**,不新增欄)② 寫異常**三類**:`tw-no-stock`(三家沒貨)/ `tw-below-low-sales`(湊不滿低銷)/ `tw-data-gap`(已分配但缺單價→$0、或盒裝查不到每箱數量)到 `anomalies.jsonl`,02 異常 tab 已加 TW 平台 filter + 類型 label。建單單價用「每個價」(box 依廠商還原;ERP PurchasePrice 為 0 不用)。
...
5. **不新增欄位(已修正)**:欄位由公司預先建好;工具只寫進 sheet **最右(最新)既有欄組**的「有庫存」欄(`find_latest_week`),不自動新增。
...
4. **寫回 sheet**:寫進**最右(最新)既有欄組**的「有庫存」欄 → 命中的料號打 `v`(**不自動新增欄**,欄位公司預建)。
5. 沒命中對照的料號 → 列「資料維護提示」。
