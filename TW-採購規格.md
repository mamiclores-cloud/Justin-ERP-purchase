...
> 狀態:**Phase A(庫存比對)已實作 + 端到端驗證(dry-run)**;Phase B(分配+建單)未開始。本檔彙整自客戶逐字稿(`智能採購-TW/逐字稿/00–07`)+ supplier sheet 實際欄位 + 三廠商測試素材的實證。
>
...
> **Phase B(分配+建單)已實作到「可預覽」**:`tw-purchase.js`(Node 主控:ERP `Keyword=TW` 需求 → spawn `tw/sheet_dump.py` → 以 `MainId #N` join → `lib/tw-allocate.js` 分配 → 一廠商一單 PO payload)+ `server.js` `POST /api/tw/purchase` + UI「TW 採購分配 + 建單」卡(分配預覽/建單)。分配規則:BOX 看 ERP KeyWord 標籤(箱大小取 sheet 一箱幾件)、最低量補齊後再進位 6 倍數、低銷 IL8000/HS3000/IN5000、需求全訂、湊不滿改別家。離線(真 sheet + 合成 ERP)已驗證 join/分配/payload 接線。
> **已補(execute 完整度)**:Phase B execute 會 ① 回填 sheet(`tw/sheet_writeback.py` 寫 需求量 + 各廠商採購量到當週欄組)② 把「訂不到」(`tw-no-stock` / `tw-below-low-sales`)寫入 `anomalies.jsonl`,02 異常 tab 已加 TW 平台 filter + 類型 label。box 單價依「單價≥箱數視為每箱÷箱數」還原成每個(金額/低銷/建單單價都用還原值;ERP PurchasePrice 為 0 不用)。
> **尚待(實機驗證)**:Phase A `--execute`(真打 v 到副本)、Phase B `--execute`(真建單 + 回填 + 寫異常)各實跑一次;(選)排程 tab 支援 TW(目前只有 Indo/1688)。離線 + live dry-run 皆已驗證(join 351/354)。
>
...
5. **日期欄程式自動新增**:每週新增一組欄(日期 ×(IL/HS/IN)×(有庫存 / 採購量))。
...
7. **分配自由度**(同一規格多家有貨時):分給「**這次最需要湊低銷**」的廠商。
...
- 有 `BOX` 標籤的商品:廠商只能**整箱出**;sheet 標「每箱幾件」+ 最低採購量「1box」。
...
- **TW 異常記兩類**(沿用既有 `anomalies.jsonl` + CSV 機制):
  - ① 有需求量但**三家都沒貨** → 這次訂不到
  - ② 規格只有「湊不滿低銷」的廠商有貨 → 沒人接得起 → 訂不到
...
4. **寫回 sheet**:**自動新增**「`{日期}` `{廠商}` `有庫存`」欄組 → 命中的料號打 `v`。
5. 沒命中對照的料號 → 列「資料維護提示」。
